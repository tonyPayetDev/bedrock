 ...
 * Search_Ktp
 *
 * Genere un module de recherche
 *
 * @since 1.0.0
 */
class Search_Ktp extends Widget_Base
{
    ...
    {
        return __('Search Ktp', 'elementor-custom-widget');
    }
    ...
    protected function register_controls()
    {
        ...
        $this->add_control(
            'post',
            [
                'label' => __('Id search ktp', 'elementor-custom-widget'),
        ...
        $this->add_control(
            'ekit_wb_226_url',
            array(
                'label' => esc_html__('URL search', 'elementskit-lite'),
                'type'  => Controls_Manager::URL,
                'placeholder' =>  esc_html('URL quand on va cliquer pour faire une recherche'),
                'show_label' => true ,
                'label_block' => true ,
                'show_external' => true ,
                'default' => array(
        ...
        $this->add_control(
            'redirect_contact',
            array(
                'label' => esc_html__('URL redirection contact', 'elementskit-lite'),
                'type'  => Controls_Manager::URL,
                'placeholder' =>  esc_html('Saisir URL qui redirige vers contact si aucune recherche'),
                'show_label' => true ,
                'label_block' => true ,
                'show_external' => true ,
                'default' => array(
        ...
        foreach ($opt as $opt_key => $value) {
            ...
            $repeater->add_control(
                'category_slug',
                [
                'label'       => __('Key', 'elementor'),
                'type' => Controls_Manager::SELECT,
                'default'     => __('Key', 'elementor'),
                'label_block' => true,
            ...
            $this->add_control(
                $opt_key.'categories',
                [
                'label'       => __('Filtres '.$opt_key, 'elementor'),
        ...
        foreach ($opt as $opt_key => $value) {
            ...
            $this->add_control(
                $opt_key.'services',
                [
                'label'       => __('Valeur '.$opt_key, 'elementor'),
                'type'        => Controls_Manager::REPEATER,
        ...
        $this->add_control(
            'ekit_wb_225_url',
            array(
                'label' => esc_html__('URL carte', 'elementskit-lite'),
                'type'  => Controls_Manager::URL,
                'placeholder' =>  esc_html('Saisir URL quand on clique sur la card'),
                'show_label' => true ,
                'label_block' => true ,
                'show_external' => true ,
                'default' => array(
        ...
        // va génerer les champs d'apres l'api selectionner , va generer dynamiquement les repeater , quand on selectionne le choix d'api va donner le bon reapeater
        foreach ($opt as $opt_key => $value) {
            ...
            // permet de sélectionner un champ et placer l'élément soit en haut, bas, gauche, droite
            $repeater3->add_control(
                "direction_element",
                [
            ...
            //élément si placé dans la photo
            $repeater3->add_control(
                'in_the_photo',
                [
                    'label' => esc_html__('Dans la photo ', 'elementskit-lite'),
                    'type' => Controls_Manager::SWITCHER,
                    'label_on' => esc_html__('Show', 'elementskit-lite'),
                    'label_off' => esc_html__('Hide', 'elementskit-lite'),
                    'return_value' => 'yes',
            ...
            $repeater3->add_control(
                'field_icon',
                array(
                    'label' => esc_html__($opt_key.'Icons', 'elementskit-lite'),
                    'type'  => Controls_Manager::ICONS,
                    'show_label' => true ,
                    'label_block' => true ,
                    'skin' => 'media' ,
            ...
            $this->add_control(
                $opt_key.'fields',
                [
                    'label'       => __('Cartes '.$opt_key, 'elementor'),
                    'type'        => Controls_Manager::REPEATER,
    ...
    public function get_api()
    {
        ...
        $query = new \WP_Query($request_p);
            
        foreach ($query->posts as $key => $value) {
            $meta = get_post_meta($value->ID);
            //  var_dump($meta['url']);
            $opt[$meta['name'][0]]=  esc_html__($meta['name'][0], 'elementskit-lite');
        }
    ...
    public function get_name_select($opt_key)
    {
        ...
        if (get_current_user_id()) {
            $args = array(
                'headers'     => array(
                    'Authorization' => $this->get_authorization($opt_key),
                ),
            );
            
            $response = wp_remote_get($this->get_url_api($opt_key)."&name_select=true", $args);//une erreur curl 28 apparait en localhost mais fonctionne en prod
            // Exit if error.
    ...
    public function get_url_api($opt_key)
    {
        $request_p =  array(
            'post_type' =>'apisearch',
            'posts_per_page'   => -1,//-1 all
        ...
        foreach ($query->posts as $key => $value) {
            $meta = get_post_meta($value->ID);
            //  var_dump($meta['url']);
            if ($meta['name'][0]===$opt_key) {
                return $meta['url'][0];
            }
        }
    }
    public function get_authorization($opt_key)
    {
        $request_p =  array(
            'post_type' =>'apisearch',
            'posts_per_page'   => -1,//-1 all
    ...
    protected function render($instance = [])
    {
        ...
        $url =$this->get_url_api($settings['view']);
        $authorization =$this->get_authorization($settings['view']);
        ...
        foreach ($this->get_api() as $api) {
            if ($settings[$api.'categories']) {
                foreach ($settings[$api.'categories'] as $category) {
                    $tab_value=null;
               
                    if ($settings[$api.'services']) {
                        foreach ($settings[$api.'services'] as $service) {
            ...
            if ($api_settings) {
                foreach ($api_settings as $field) {
                    $array_field= array(
                        ...
                        'in_the_photo'=> $field['in_the_photo'], //rajout
                        'direction_element'=> $field['direction_element'], // choisir la direction
        ...
        // affiche une simple preview
        if (get_current_user_id()) {
            $context['heading_text']=$array['heading_text'];
            $context['color']=$array['color'];
            $context['search_text']=$array['search_text'];
            $context['ekit_wb_3976_font']=$array['ekit_wb_3976_font'];
            $context['post']=$array['post'];
